...
([planning.py:88](../../semantic_layer/engine/planning.py#L88)), right after
...
([base.py:47](../../semantic_layer/engine/base.py#L47)). This simultaneously:
...
- [ ] **1.2** Add optional kill-switch config `wren_dialect_finalize_enabled: bool = True`
  ([config.py](../../config.py), near `wren_engine`) + `WREN_DIALECT_FINALIZE_ENABLED`
...
- [ ] **2.1** In `plan_semantic_sql_step` ([planning.py:88](../../semantic_layer/engine/planning.py#L88)),
...
- [ ] **4.1** Backend: `evaluate_semantic_factors`
  ([mode.py](../../semantic_layer/engine/mode.py)) — when the backend is in
