...
**Status:** IMPLEMENTED (P1–P5 + gap closure), tested, in the working tree. See **`MDL_LAB.md`** for the as-built reference (what exists, symbols, tests, residual gaps). This document remains the rationale/decision record.
...
- The **Copilot is hard-gated behind onboarding**: `isReady = readinessStatus === 'ready'` hides the entire chat until active models exist ([CopilotPanel.tsx](../../../superset-frontend/src/SqlLab/components/AiAgentPanel/SemanticLayerEditor/CopilotPanel.tsx), backend 409 gate [app.py:1705-1718](../../app.py#L1705-L1718)). The agent cannot help you *get* onboarded; you must onboard a whole schema manually first.
- **Access is ownership-based**: the project creator becomes `admin`; everyone else with DB access gets `read` (or `write` behind a flag) ([projects.py](../../semantic_layer/projects.py) `_with_permission`/`_is_visible`, [access.py](../../semantic_layer/access.py) `_project_with_permission`). This conflicts with the intended model where a project belongs to a **database**, not a person.
- **Project identity is `(fingerprint, catalog, schema, deleted_at)` unique** ([persistence/models.py:216-228](../../persistence/models.py#L216-L228)), so a database can hold exactly one project per schema and a project **cannot be duplicated** within the same scope.
...
- **Re-scope owner-filtered sub-objects to the project (security-critical, DP3):** documents, document chunks, and events are currently filtered by `owner_id` ([sqlalchemy_store.py]). For a shared project these must be filtered by **`project_id`** so every DB-authorized user sees the project's full context. The project is DB-fingerprint-bound, so project-scoping never widens beyond the DB boundary. **NL→SQL memory AND Copilot instructions stay `owner_id`-scoped** (personal guidance/learning, not shared project context) — DP3 (revised, DP-NEW). Because instructions diverge from the otherwise project-shared context, the **UI must clearly tell the user their instructions are personal** (not shared with other DB-authorized users) — see §5.4a.
...
### 5.4a Personal Copilot instructions (DP-NEW, resolved)

Copilot instructions ([instructions.py]) remain **personal** — scoped by `owner_id` + `scope_hash`, exactly like NL→SQL memory — because they are personal guidance to the agent, not shared project context. This is a deliberate divergence from the otherwise project-shared sub-objects (docs/events/coverage/RAG), and was confirmed with the requester (DP-NEW = keep personal). To prevent surprise under sharing, the **instructions UI carries a clear, persistent note** that instructions are personal to the current user and are not shared with other users who have access to the project's database. No data/scope change; documentation + a single UI affordance only.

### 5.5 Copilot scope expansion (F4)

- **Remove the readiness gate** in the UI (`isReady` no longer hides the chat) and the backend 409 ([app.py:1705-1718](../../app.py#L1705-L1718)). An empty project opens straight into a usable Copilot with onboarding affordances.
...
- **Vector store is already project-scoped, no owner in the key:** `document_scope_key(project_id) → "doc:{project_id}"` ([document_retriever.py:59-71](../../semantic_layer/document_retriever.py#L59-L71)). Vectors for every uploader's chunks live under the one project key. ✅
- **But the retrieval *corpus* is owner-filtered:** the toolset feeds `retrieve()` the candidate set `list_project_chunks(project_id, owner_id=self._owner_id)` ([copilot/tools.py:374](../../semantic_layer/copilot/tools.py#L374)), and `retrieve()` **intersects** vector hits back to that set (`ordered = [by_id[id] for id in ids if id in by_id]`, [document_retriever.py:157-158](../../semantic_layer/document_retriever.py#L157-L158)). So a project-wide vector hit on **user A's** doc is dropped when **user B** searches. The keyword fallback (`keyword_rank_chunks(query, chunks, k)`) ranks the same owner-filtered corpus. **Net: retrieval is effectively owner-scoped today.**
- **`list_project_documents`** (the agent's `list_documents` tool and coverage's doc set) is owner-filtered too ([sqlalchemy_store.py](../../semantic_layer/sqlalchemy_store.py)).
...
| **DP3** | Re-scoping owner-filtered sub-objects | (a) documents+events → project-scoped, memory **and instructions** stay owner-scoped; (b) everything project-scoped; (c) leave owner-scoped | **(a)** (revised by DP-NEW). Project context (docs/events) must be shared across DB-authorized users; personal NL→SQL memory **and Copilot instructions** stay per-user (personal guidance, not shared context) — the instructions UI states this explicitly (§5.4a). (c) breaks F5 (a second user sees an empty project). |
| **DP-NEW** | Copilot instruction scope under sharing | (a) keep personal + explain in UI; (b) project-scope; (c) two tiers | **(a)** (confirmed). Instructions are personal guidance like NL→SQL memory; keeping them per-user avoids a behaviour change + per-row migration. UI must clearly label them personal (§5.4a). |
