...
- **Code.** Every `MdlModel` already carries its own physical location via `MdlTableReference { catalog, schema, table }` ([mdl_schema.py:81-102](../../semantic_layer/mdl_schema.py#L81-L102)). The manifest's *root* `catalog`/`schema` ([mdl_schema.py:162-163](../../semantic_layer/mdl_schema.py#L162-L163), [mdl_compile.py:58-59](../../semantic_layer/mdl_compile.py#L58-L59)) is wren-core's **logical namespace**, not a filter on which physical tables models may reference. `compile_manifest` merges per-model bodies through unchanged ([mdl_compile.py:98-127](../../semantic_layer/mdl_compile.py#L98-L127)) — nothing collapses or rewrites `tableReference.schema`.
...
Note one layer **already anticipated** multi-schema: `SemanticAccessProof.schema_names: list[str]` ([access.py:60](../../semantic_layer/access.py#L60)). The proof is plural; only the project and scope are scalar.
...
- `onboard_schema_project` generalizes to accept an `OnboardingSelection` whose table entries are **schema-qualified** (`{schema, table}` pairs), spanning any subset of the project's schema set ([onboarding.py:59-147](../../semantic_layer/onboarding.py#L59-L147)).
...
- `ConversationScope.schema_name: str` → `schema_names: list[str]` (back-compat coercion). `scope_hash` ([store.py](../../semantic_layer/store.py)) incorporates the **sorted** schema set so cache identity is stable regardless of order.
- `resolve_project` / `_require_project_permission` must prove access to **every** schema in the set, not just one ([access.py:120-129, 205-214](../../semantic_layer/access.py#L120-L214)). `SemanticAccessProof.schema_names` is already plural — wire it through.
...
- `validate_project_manifest` / `SchemaIndex` resolve a model's table against the **schema named in its `tableReference`** ([mdl_validator.py](../../semantic_layer/mdl_validator.py)). Error text becomes schema-qualified: `Model X references table 'crm.customers' that does not exist in any project schema.`
...
- `materialize_wren_project` keeps writing **one** manifest. Root `catalog`/`schema` become the fixed logical namespace (project id-derived), **not** a physical schema ([wren_materializer.py:58-117](../../semantic_layer/wren_materializer.py#L58-L117)). Per-model `tableReference.schema` already carries physical location, so the compiled manifest is unchanged in shape — only the root namespace semantics are clarified.
...
- `MdlToolset` receives the unioned multi-schema `SchemaIndex`; `get_schema` retrieval ranks across all schemas ([copilot/tools.py](../../semantic_layer/copilot/tools.py), [schema_retriever.py](../../semantic_layer/schema_retriever.py)). `SchemaItem` gains a `schema` qualifier so retrieved tables are unambiguous and the agent emits correct `tableReference.schema`.
...
| **D4** | Table name collisions across schemas (`sales.orders` vs `archive.orders`) | (a) key `SchemaIndex` by `(schema, table)`, require model names unique; (b) auto-prefix model names by schema | **(a)**. Keep the index `(schema, table)`-keyed so both physical tables are addressable; model **logical** names must still be unique within the manifest (wren-core requirement — see dedupe note [mdl_compile.py:154-162](../../semantic_layer/mdl_compile.py#L154-L162)). When onboarding would create two models named `orders`, disambiguate the *logical* name (e.g. `sales_orders`) while `tableReference` stays exact. Don't force prefixes when there's no collision. |
