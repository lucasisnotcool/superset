# PROVENANCE — verbatim upstream Wren SDK file, fetched for reference only (NOT committed as runtime).
# Source: sdk/wren-langchain/src/wren_langchain/_prompt.py · Canner/WrenAI @ main · fetched 2026-06.
# Closest architectural match to OUR query agent: Wren's LangChain/LangGraph system-prompt
# BUILDER. It distills the `usage` skill into a system prompt (recall → fetch context →
# compose SQL → dry_plan if complex → execute → store; phase-based error recovery; strong
# "by default" phrasing; prompt kept in sync with the tool list). NOTE: wren-langchain is
# TOOL-CALLING; our query agent is a STRUCTURED-OUTPUT LangGraph pipeline — map its workflow
# steps onto our graph NODES, not onto tools.
