<!--
PROVENANCE — verbatim upstream Wren skill, fetched for reference only (NOT committed as runtime).
Source: core/wren/src/wren/skills_content/usage/SKILL.md · Canner/WrenAI @ main · fetched 2026-06.
This is the day-to-day NL→SQL WORKFLOW (a CLI-host agent playbook) — the methodology
source for OUR query agent (graph.py / conversation_graph.py). It is procedural
orchestration, not an LLM system prompt: port its METHODOLOGY into our graph nodes,
not its `wren` CLI commands. See wren_langchain_prompt.py for the system-prompt form.
-->
---
name: usage
