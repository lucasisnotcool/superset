...
Source: https://raw.githubusercontent.com/Canner/WrenAI/main/core/wren/src/wren/skills_content/generate-mdl/SKILL.md
Repo: Canner/WrenAI @ main · path: core/wren/src/wren/skills_content/generate-mdl/SKILL.md
Fetched: 2026-06 (verify currency before relying on it).
This is the genuine source for tailoring superset_ai_agent/skills/generate-mdl.md.
NOTE: upstream authors snake_case YAML compiled to camelCase; OUR copilot authors JSON
camelCase directly (see mdl_schema.py / mdl_copilot.md). Reconcile this when tailoring.
-->
...
> When to define cubes: If the user asks aggregation questions like "revenue by month" or "top customers", define cubes alongside models — they give agents a structured query API instead of forcing them to hand-write `GROUP BY` / `DATE_TRUNC` SQL.
...
After this step, `wren memory fetch` and `wren memory recall` are operational. See `wren skills get usage` for query workflows.
...
Each change follows: edit YAML → `wren context validate` → `wren context build` → `wren memory index`.
