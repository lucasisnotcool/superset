...
- **Physical catalog source.** `SchemaIndex` is built from Superset *datasets* when they exist; when a schema has none, the modeling-time provider (`context/superset_metadata.py::get_full_schema`) falls back to **live DB introspection** (Superset `/tables/` + `/table_metadata/`, owner-scoped) so a BYO / live connection + MDL project onboards without registering datasets. Gated by `wren_live_schema_introspection` (default on). See [plans/plan_live_schema_introspection_spec.md](../plans/plan_live_schema_introspection_spec.md).
