<!--
PROVENANCE — verbatim upstream Wren scaffold template, fetched for reference only (NOT committed as runtime).
Source: core/wren/src/wren/context.py, constant `_AGENTS_MD_TEMPLATE`
Repo: Canner/WrenAI @ main · written into each project root by `wren context init`.
Fetched: 2026-06.

This is the closest Wren equivalent to a *base agent/system prompt*: the per-project
"how to work with this project" instructions the host agent (Claude Code / Cursor) reads.
It is CLI-centric (the wren CLI). For OUR fork it is a STRUCTURAL baseline for
prompts/mdl_copilot.md — adopt its skeleton (project intro → answering data questions →
modifying the data model → prerequisites → quick reference), NOT its CLI commands.
The "Modifying the data model" section is the part that maps to the MDL Copilot;
"Answering data questions" maps to our separate query agent (out of scope here).
-->
