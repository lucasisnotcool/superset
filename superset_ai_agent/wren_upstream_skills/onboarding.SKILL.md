...
Source: https://raw.githubusercontent.com/Canner/WrenAI/main/core/wren/src/wren/skills_content/onboarding/SKILL.md
Repo: Canner/WrenAI @ main · path: core/wren/src/wren/skills_content/onboarding/SKILL.md
Fetched: 2026-06 (verify currency before relying on it).
This is the genuine source for tailoring superset_ai_agent/skills/onboarding.md.
Most CLI/.env/profile content does NOT apply to our stack (no `wren` CLI; onboarding is
deterministic Python over Superset datasets). Keep the *behavioral* rules, drop the CLI.
-->
...
- `docs/core/get_started/installation.md` — CLI install + skill install
- `docs/core/guides/connect.md` — full connection procedure, per-datasource setup notes, complete troubleshooting playbook
- `docs/core/get_started/quickstart.md` — bundled `jaffle_shop` demo
...
2. `pip install "wrenai[<ds>,main]"`. For datasource-specific install gotchas (macOS mysql, etc.), see `connect.md#per-datasource-setup-notes`.
...
> The agent must build MDL before any data query. Queries against tables not in MDL will fail.
...
If you hit something not in the playbook, tell the user to see the Wren docs or open an issue.
