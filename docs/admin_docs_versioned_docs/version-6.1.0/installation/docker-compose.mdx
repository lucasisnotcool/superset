...
```bash
# Single lightweight instance (default port 8092)
docker compose -f docker-compose-light.yml up

# Multiple instances with different ports:
# copy docker/.env-local.example for each clone, edit NODE_HOST_PORT, then pass it
...
Access each instance at `http://localhost:{NODE_HOST_PORT}` (for example,
`http://localhost:8092`).
...
web browser by visiting `http://localhost:8092`. Note that many browsers now default to `https` - if
