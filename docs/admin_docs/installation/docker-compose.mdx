...
```bash
# Single lightweight instance (default port 8090)
docker compose -f docker-compose-light.yml up

# Multiple instances with different ports:
# copy docker/.env-local.example for each clone, edit NGINX_HOST_PORT, then pass it
# with --env-file docker/.env-local.
```
...
Access each instance at `http://localhost:{NGINX_HOST_PORT}` (for example,
`http://localhost:8090`).
...
web browser by visiting `http://localhost:8090`. Note that many browsers now default to `https` - if
