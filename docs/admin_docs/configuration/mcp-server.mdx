...
```mermaid
flowchart LR
    A["AI Client<br/>(Claude, ChatGPT, etc.)"] -- "MCP protocol<br/>(HTTP + JSON-RPC)" --> B["MCP Server<br/>(:8098/mcp)"]
...
```bash
superset mcp run --host 127.0.0.1 --port 8098
```

| Flag | Default | Description |
|------|---------|-------------|
| `--host` | `127.0.0.1` | Host to bind to |
| `--port` | `8098` | Port to bind to |
  ...
  "mcpServers": {
    "superset": {
      ...
      "args": [
        "-y",
        "mcp-remote@latest",
        "http://your-superset-host:8098/mcp",
        "--header",
...
services:
  ...
  mcp:
    image: apache/superset:latest
    command: ["superset", "mcp", "run", "--host", "0.0.0.0", "--port", "8098"]
    ports:
      - "8098:8098"
    volumes:
      - ./superset_config.py:/app/superset_config.py
...
| `MCP_SERVICE_PORT` | `8098` | Port the MCP server binds to |
...
- Confirm the port is not already in use: `lsof -i :8098`
...
- Confirm the URL path ends with `/mcp` (e.g., `http://localhost:8098/mcp`)
